...
### <a name="requirements"></a>需求  
 ...
 可能的話，您的資料也應包含有時間或日期序列的資料行。 您可以使用數位序列 (1、2、3 .... ) ，而不是日期和時間資料。 不過，序列資料行中的值必須是唯一的。 如果**預測**工具在數列資料行中找到重複的值，就會發生錯誤。  
...
## <a name="related-tools"></a>相關工具  
 ...
 [適用于 Excel 的資料表分析工具]) 中 (的 [**預測**] 工具，以及適用于 Excel 的資料採礦用戶端 (的 [**預測**]) 使用 [!INCLUDE[msCoName](../includes/msconame-md.md)] 時間序列演算法。  
