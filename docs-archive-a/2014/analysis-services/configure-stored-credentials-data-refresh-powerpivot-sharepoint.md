...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入 PowerPivot 服務應用程式的應用程式集區識別，讓它能夠在資料重新整理排程為執行時擷取認證。 若要查看應用程式集區身分識別，請移至 [**管理服務應用程式**]，按一下 [名稱] 旁邊的空白處，選取 [PowerPivot 服務應用程式] (這會選取資料列) ，然後按一下 [**屬性**]。 出現在此頁面的安全性帳戶就是要當做目標應用程式成員加入的帳戶。  
...
2.  確認已在 [資料重新整理設定] 頁面中，啟用 [**使用儲存在 Secure Store Service (SSS) 中的認證來登入資料來源]** 選項，然後輸入目標應用程式的名稱。  
...
13. 在 [成員] 中，加入下列使用者和群組帳戶：  
    ...
    2.  加入將使用目標應用程式存取其預存認證之每個 PowerPivot 服務應用程式的應用程式集區識別。 若要查看身分識別，請移至 [**管理服務應用程式**]，按一下 [名稱] 旁邊的空白處來選取 PowerPivot 服務應用程式 (這會選取資料列) ，然後按一下 [**屬性**]。 出現在此頁面的安全性帳戶就是您要當做目標應用程式成員加入的帳戶。  
