 ...
 **資料分割部署選項**  
 \<*project name*>D 檔案會指定是否保留或覆寫目的地資料庫中的現有分割區 (預設) 。 如果保留現有的資料分割，則只會部署新的資料分割，而所有現有的量值群組上的資料分割與彙總設計都會保持不變。  
...
-   在命令提示字元下執行 [ [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 部署精靈]，並將精靈設定為以回應檔案模式執行。  (需回應檔案模式的詳細資訊，請參閱執行[Analysis Services 部署嚮導](running-the-analysis-services-deployment-wizard.md)。 )   
