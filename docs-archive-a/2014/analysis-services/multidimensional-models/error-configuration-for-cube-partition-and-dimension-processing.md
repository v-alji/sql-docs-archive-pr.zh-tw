...
##  <a name="missing-keys-keynotfound"></a><a name="bkmk_missing"></a> (KeyNotFound) 遺漏金鑰  
...
4.  將值變更為 `Error` ，以排除具有 null 值的任何記錄，避免 null 到數值的 (零) 轉換。 這項修改可讓您避免與索引鍵資料行中具有零的多個記錄相關的重複索引鍵錯誤，也可以避免 `KeyNotFound` 在相關維度資料表中有零值外鍵沒有對等的主鍵時發生錯誤。  
