...
|**#**|代表會顯示數字或零的數字預留位置。<br /><br /> 如果運算式在格式字串中出現數位記號 () 的數位 **#** ，則格式化的值會顯示數位。 否則，格式值就會在那個位置顯示一個零。<br /><br /> 數位記號 (**#**) 預留位置的運作方式就像零 (**0**) 數位預留位置，不同的是，如果數位的位數與 **#** 格式運算式中小數點兩邊的字元數相同或較少，則不會顯示前置和尾端零。|  
|**.**|代表可決定在小數點左、右要顯示幾位數字的小數預留位置。<br /><br /> 如果格式運算式僅包含數位記號 (**#** (**.**) 的句點左邊) 個字元，小於1的數位就會以小數分隔符號開頭。 若要在小數點數字開頭顯示一個零，請使用零 (0) 做為小數點左邊的第一個數字預留位置。<br /><br /> 在格式化輸出中實際做為小數預留位置的字元，將依電腦系統所能辨識的數字格式而有所不同。<br /><br /> 注意：在某些地區設定中，是用逗點做為小數點。|  
...
|**-+ $ ( ) **|顯示實際字元。<br /><br /> 若要顯示其中一個以外的字元，請在字元前面加上一個反斜線 (**\\**) ，或用雙引號括住字元 (**""**) 。|  
...
|**d**|將日期顯示為沒有前置零 (1-31) 的數位。|  
|**dd**|將日期顯示為具有前置零 (01-31) 的數位。|  
|**ddd**|將日期顯示為 (Sun-Sat) 的縮寫。|  
|**dddd**|將日期顯示為 (星期日-星期六) 的完整名稱。|  
...
|**ww**|將年份的周顯示為數字 (1-54) 。|  
|**分鐘**|將月份顯示為沒有前置零 (1-12) 的數位。<br /><br /> 如果 **m** 是直接跟隨在 **h** 或 **hh**後面，則顯示的將是「分」而不是「月」。|  
|**mm**|將月份顯示為有前置零 (01-12) 的數位。<br /><br /> 如果 **m** 是直接跟隨在 **h** 或 **hh**後面，則顯示的將是「分」而不是「月」。|  
|**mmm**|將月份顯示為縮寫 (Jan-Dec) 。|  
|**mmmm**|將月份顯示為完整月份名稱 (1 月-12 月) 。|  
|**問題解答**|將一年的季顯示為數字 (1-4) 。|  
|**y**|將一年中的日期顯示為 (1-366) 的數位。|  
