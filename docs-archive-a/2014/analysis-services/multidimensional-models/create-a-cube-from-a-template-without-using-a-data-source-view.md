...
## <a name="selecting-dimensions"></a>選取維度  
 ...
 若要加入現有的維度，請在 [共用維度]**** 清單中選取一個或多個維度，然後按一下向右箭頭 (**>**) 按鈕將其移至 [Cube 維度]**** 清單中。 按一下雙箭號 (**>>**) ] 按鈕，以移動清單中的所有維度。  
...
>  只有當您在精靈的 [定義新的維度]**** 頁面上，將時間維度加入 Cube 時，才會顯示 [定義時間週期]**** 頁面。  
 ...
 維度設計師的 [時間週期] 窗格中會顯示所有可用的時間週期  (此窗格會取代以維度資料表為基礎之維度的 [**資料來源] 視圖**窗格。 ) 您可以變更維度的 [**來源** (時間系結]) 屬性設定，以變更維度的日期範圍。 由於這是結構性變更，因此您必須重新處理維度及任何使用維度的 Cube，再瀏覽資料。  
