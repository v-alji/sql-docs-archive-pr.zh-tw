...
> [!NOTE]  
>  如果您定義了空的 set ({} 屬性成員) ，資料庫角色就看不到該屬性的任何成員。 沒有允許的集合不會被解釋為空的集合。  
 ...
 **啟用視覺化總計**  
 ...
 根據預設，VisualTotals 屬性會停用 (設定為 [ `False`) ]。 此預設值會使效能最佳化，因為 [!INCLUDE[ssASnoversion](../../includes/ssasnoversion-md.md)] 可快速計算所有資料格的值之總計，而不必浪費時間選取要計算之資料格的值。  
