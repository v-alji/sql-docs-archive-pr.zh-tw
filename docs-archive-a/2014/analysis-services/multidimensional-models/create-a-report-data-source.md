...
5.  在 [連接字串]**** 中，指定 Analysis Services 伺服器名稱、資料庫名稱、Cube 名稱及任何選擇性設定。  
    ...
    >  如果有多個 Cube，則必須指定 Cube 名稱。  
     ...
     (選擇性) 在模型中，可以指定各種語言的 Cube 中繼資料和資料翻譯。 若要查看翻譯 (的資料和中繼資料) 您必須將 [地區設定識別碼] 屬性加入至連接字串：`Data source=<servername>;initial catalog=<multidimensionaldatabasename>-ee;cube='<cubename>'; Locale Identifier=<identifier number>`  
