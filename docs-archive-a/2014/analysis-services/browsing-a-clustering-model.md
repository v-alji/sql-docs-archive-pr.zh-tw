...
5.  現在，按一下 [**陰影變數**] 下拉式清單，然後選擇 [ **Age** ] 變數。 當您這麼做時，[**狀態**] 文字方塊中會出現值清單。  
     ...
     當做此模型之輸入使用的 Age 資料行包含連續數值，但是基於叢集的目的，演算法一定會離散化數字。 您可以在這裡看到此演算法所建立的 bin 或群組，例如「非常低的 (\<=27)" and "Very High (> = 63) 」。  
...
2.  現在，將游標移至叢集設定檔中的 Occupation 上方。  
     ...
     請注意，在某些叢集 (（例如圖形) 中的），職業清單並不完整，而且某些職業會取代為標籤 [**其他**]。  
...
-   以滑鼠右鍵按一下叢集的資料行標題，然後選取 [**鑽取至結構資料**]，以取得模型中未包含之叢集成員的詳細資訊。  
     ...
     例如，如果您要分析客戶，您可能會將基礎資料中的連絡人資訊保留 (的採礦結構) 但不會將它包含在模型中，因為它不適用於分析。 不過，將客戶指派給叢集之後，您就可以使用鑽研來檢視詳細資料。  
