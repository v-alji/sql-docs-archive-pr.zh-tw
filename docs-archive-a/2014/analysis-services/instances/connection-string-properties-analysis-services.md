...
|`Connect Timeout`|決定用戶端在超時前嘗試連線的時間量上限 (秒數) 。如果連線在這段期間內未成功，用戶端會結束嘗試連線並產生錯誤。|  
 ...
 **伺服器的原生 (或直接) 連接**  
 ...
 `Data Source=server[:port][\instance]`其中 "port" 和 "\instance" 是選擇性的。 例如，指定 "Data Source = server1" 會開啟與預設實例的連接 (和名為 "server1" 之伺服器上的預設通訊埠 2383) 。  
  
 "Data Source = server1： port1" 將會開啟與 "server1" 上端口 "port1" 上執行之 Analysis Services 實例的連接。  
  
 "Data Source = server1\instance1" 將會開啟 SQL Browser (在其預設通訊埠 2382) 上的連接、解析已命名實例 "instance1" 的埠，然後開啟該 Analysis Services 埠的連線。  
