...
##  <a name="use-a-fixed-port-for-a-default-or-named-instance-of-analysis-services"></a><a name="bkmk_fixed"></a>針對預設或名為的實例使用固定通訊埠 Analysis Services  
 ...
 如果您使用的是已命名的實例，您可以指派固定通訊埠，而不會變更指定伺服器名稱的方式 (具體來說，您可以使用 \<servername\instancename> 連接到接聽固定通訊埠) 的已命名實例。 SQL Server Browser 服務必須在執行中，才可解除對其所接聽之通訊埠的封鎖。 SQL Server Browser 服務會根據來提供重新導向至固定通訊埠 \<servername\instancename> 。 只要您開啟通訊埠供 SQL Server Browser 服務和接聽固定通訊埠的 Analysis Services 具名執行個體使用，SQL Server Browser 服務就會將連線解析為具名執行個體。  
