...
3.  認證物件所提供的使用者名稱和密碼會解析 Windows 使用者識別。 Analysis Services 會以目前使用者的身分使用此識別。 如果使用者不是 Windows 使用者，或缺少執行所要求之作業的足夠權限，則要求將會失敗。  
 ...
 若要建立認證物件，您可以使用 Get-Credential 指令程式，向操作員收集認證。 接著，您可以在命令中使用連接至 Analysis Services 的認證物件。 下列範例將說明其中一個方法。 在此範例中，連接至本機實例 (`SQLSERVER:\SQLAS\HTTP_DS`) 設定 HTTP 存取。  
...
```  
 ...
 使用基本驗證時，您應該一律搭配 SSL 使用 HTTPS，才能透過加密的連接傳送使用者名稱和密碼。 如需詳細資訊，請參閱[在 iis 7.0 中設定安全通訊端層](https://go.microsoft.com/fwlink/?linkID=184299)和[ (Iis 7) 設定基本驗證](https://go.microsoft.com/fwlink/?LinkId=230776)。  
