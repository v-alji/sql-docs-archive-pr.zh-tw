...
> [!IMPORTANT]  
>  將通訊協定前置詞與 `Server` 屬性結合 (`Server=tcp:` *\<servername>*) 與**Network**屬性不相容，而且在兩個位置指定通訊協定可能會導致錯誤。 因此，我們建議連接字串使用**Network**屬性來指定通訊協定，並且只在 `Server` 屬性 () 中指定伺服器名稱 `"Network=dbmssocn; Server=` *\<servername>* `"` 。  
