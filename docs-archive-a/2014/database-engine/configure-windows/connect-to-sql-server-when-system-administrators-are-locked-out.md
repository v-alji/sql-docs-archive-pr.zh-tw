...
8.  在 Windows 8 的 [開始] 畫面上，以滑鼠右鍵按一下 [!INCLUDE[ssManStudio](../../includes/ssmanstudio-md.md)] 的圖示。 在畫面底部，選取 [以系統管理員身分執行]**** (這樣會將您的系統管理員認證傳遞給 SSMS)。  
    ...
    2.  不透過 [物件總管] 連接，而是使用 Windows 驗證 (包含您的系統管理員認證) 透過查詢視窗連接  (只有在您未使用物件總管連接時，才能以這種方式連接。 ) 執行下列程式碼來加入屬於固定伺服器角色成員的新 Windows 驗證登入 `sysadmin` 。 下列範例會加入名為 `CONTOSO\PatK` 的網域使用者。  
    ...
    4.  如果您的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 是以混合驗證模式執行，而您想要重設帳戶的密碼 `sa` ，請使用 Windows 驗證（包含您的系統管理員認證)  (）與查詢視窗連接。 `sa`使用下列語法變更帳戶的密碼。  
