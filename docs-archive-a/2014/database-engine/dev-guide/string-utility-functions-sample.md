...
ms.locfileid: "87701725"
---
# <a name="string-utility-functions-sample"></a><span data-ttu-id="2c30f-102">字串公用程式函數範例</span><span class="sxs-lookup"><span data-stu-id="2c30f-102">String Utility Functions Sample</span></span>
  <span data-ttu-id="2c30f-103">這個「字串公用程式」範例包含以 Visual C# 和 Visual Basic 撰寫的資料流資料表值函式，它會將逗號分隔字串分割成具有一個資料行的資料表。</span><span class="sxs-lookup"><span data-stu-id="2c30f-103">The String Utilities sample contains a streaming table-valued function written in Visual C# and Visual Basic, which splits a comma-separated string into a table with one column.</span></span> <span data-ttu-id="2c30f-104">它也包含彙總函式，可將字串資料行轉換為逗號分隔字串。</span><span class="sxs-lookup"><span data-stu-id="2c30f-104">It also contains an aggregate function that converts a string column to a comma-separated string.</span></span>  <span data-ttu-id="2c30f-105">另外還實作純量函數和資料表值函式，它們提供規則運算式取代和規則運算式搜尋功能。</span><span class="sxs-lookup"><span data-stu-id="2c30f-105">Also implemented are a scalar function and a table-valued function that provide regular expression replacement and regular expression searching functionality.</span></span>  
  
 <span data-ttu-id="2c30f-106">若要實作資料流資料表值函式，請建立傳回實作 `IEnumerable` 介面之物件的方法。</span><span class="sxs-lookup"><span data-stu-id="2c30f-106">To implement a streaming table-valued function, create a method that returns an object that implements the `IEnumerable` interface.</span></span> <span data-ttu-id="2c30f-107">此 `IEnumerable` 方法必須透過屬性與填入資料表值函式資料列的另一個方法連結。</span><span class="sxs-lookup"><span data-stu-id="2c30f-107">This `IEnumerable` method must be linked by an attribute to another method which fills in rows of the table-valued function.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="2c30f-108">Prerequisites</span><span class="sxs-lookup"><span data-stu-id="2c30f-108">Prerequisites</span></span>  
 <span data-ttu-id="2c30f-109">若要建立並執行這個專案，您必須安裝下列軟體：</span><span class="sxs-lookup"><span data-stu-id="2c30f-109">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="2c30f-110">或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="2c30f-110">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="2c30f-111">您可以從 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 文件集和範例[網站](https://www.microsoft.com/download/details.aspx?id=42299)免費取得 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="2c30f-111">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/download/details.aspx?id=42299)</span></span>  
  
-   <span data-ttu-id="2c30f-112">您可以從 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 開發人員[網站](https://archive.codeplex.com/?p=SqlServerSamples)取得 AdventureWorks 資料庫</span><span class="sxs-lookup"><span data-stu-id="2c30f-112">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://archive.codeplex.com/?p=SqlServerSamples)</span></span>  
  
-   <span data-ttu-id="2c30f-113">.NET Framework SDK 2.0 或更新版本或是 Microsoft Visual Studio 2005 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="2c30f-113">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="2c30f-114">您可以免費取得 .NET Framework SDK。</span><span class="sxs-lookup"><span data-stu-id="2c30f-114">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="2c30f-115">此外，您也必須符合下列條件：</span><span class="sxs-lookup"><span data-stu-id="2c30f-115">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="2c30f-116">您所使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體必須啟用 CLR 整合。</span><span class="sxs-lookup"><span data-stu-id="2c30f-116">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="2c30f-117">若要啟用 CLR 整合，請執行下列步驟：</span><span class="sxs-lookup"><span data-stu-id="2c30f-117">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="2c30f-118">啟用 CLR 整合</span><span class="sxs-lookup"><span data-stu-id="2c30f-118">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="2c30f-119">執行下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令：</span><span class="sxs-lookup"><span data-stu-id="2c30f-119">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="2c30f-120">若要啟用 CLR 整合，您必須擁有 `ALTER SETTINGS` 伺服器層級權限，此權限是由系統管理員 (`sysadmin`) 和伺服器管理員 (`serveradmin`) 固定伺服器角色的成員以隱含方式持有。</span><span class="sxs-lookup"><span data-stu-id="2c30f-120">To enable CLR integration, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="2c30f-121">AdventureWorks 資料庫必須安裝在您所使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體上。</span><span class="sxs-lookup"><span data-stu-id="2c30f-121">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="2c30f-122">如果您不是正在使用之 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體的管理員，則必須讓管理員授與您 **CreateAssembly** 權限來完成安裝。</span><span class="sxs-lookup"><span data-stu-id="2c30f-122">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="2c30f-123">建立範例</span><span class="sxs-lookup"><span data-stu-id="2c30f-123">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="2c30f-124">使用下列指示來建立並執行範例：</span><span class="sxs-lookup"><span data-stu-id="2c30f-124">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="2c30f-125">開啟 Visual Studio 或 .NET Framework 命令提示字元。</span><span class="sxs-lookup"><span data-stu-id="2c30f-125">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="2c30f-126">必要時，請建立範例的目錄。</span><span class="sxs-lookup"><span data-stu-id="2c30f-126">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="2c30f-127">在此範例中，我們將使用 C:\MySample。</span><span class="sxs-lookup"><span data-stu-id="2c30f-127">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="2c30f-128">在 c:\MySample 中，建立 `StringUtils.vb` (適用於 Visual Basic 範例) 或 `StringUtils.cs` (適用於 C# 範例) 並將適當的 Visual Basic 或 C# 範例程式碼 (下面) 複製到檔案中。</span><span class="sxs-lookup"><span data-stu-id="2c30f-128">In c:\MySample, create `StringUtils.vb` (for the Visual Basic sample) or `StringUtils.cs` (for the C# sample) and copy the appropriate Visual Basic or C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="2c30f-129">根據您選擇的語言，在命令列提示字元中執行下列其中一段程式碼，藉以編譯範例程式碼。</span><span class="sxs-lookup"><span data-stu-id="2c30f-129">Compile the sample code from the command line prompt by executing one of the following, depending on your choice of language.</span></span>  
...
5.  <span data-ttu-id="2c30f-130">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安裝程式碼複製到檔案中，並將它儲存成範例目錄中的 `Install.sql`。</span><span class="sxs-lookup"><span data-stu-id="2c30f-130">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
6.  <span data-ttu-id="2c30f-131">如果此範例安裝在 `C:\MySample\` 以外的目錄中，請依照指示編輯 `Install.sql` 檔案，以指向該位置。</span><span class="sxs-lookup"><span data-stu-id="2c30f-131">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
7.  <span data-ttu-id="2c30f-132">部署組件和預存程序，方法是執行</span><span class="sxs-lookup"><span data-stu-id="2c30f-132">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
    -   <span data-ttu-id="2c30f-133">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 測試命令腳本複製到檔案中，並將它儲存成 `test.sql` 範例目錄中的。</span><span class="sxs-lookup"><span data-stu-id="2c30f-133">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
8.  <span data-ttu-id="2c30f-134">使用下列命令來執行測試指令碼</span><span class="sxs-lookup"><span data-stu-id="2c30f-134">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
9. <span data-ttu-id="2c30f-135">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 清除指令碼複製到檔案中，並將它儲存成範例目錄中的 `cleanup.sql`。</span><span class="sxs-lookup"><span data-stu-id="2c30f-135">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
10. <span data-ttu-id="2c30f-136">使用下列命令來執行指令碼</span><span class="sxs-lookup"><span data-stu-id="2c30f-136">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
  
## <a name="sample-code"></a><span data-ttu-id="2c30f-137">範例程式碼</span><span class="sxs-lookup"><span data-stu-id="2c30f-137">Sample Code</span></span>  
 <span data-ttu-id="2c30f-138">下面是此範例的程式碼清單。</span><span class="sxs-lookup"><span data-stu-id="2c30f-138">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="2c30f-139">C#</span><span class="sxs-lookup"><span data-stu-id="2c30f-139">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="2c30f-140">VB.NET</span><span class="sxs-lookup"><span data-stu-id="2c30f-140">VB.NET</span></span>  
...
```  
 ...
 <span data-ttu-id="2c30f-141">這是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安裝指令碼 (`Install.sql`)，它會部署組件並在資料庫中建立預存程序。</span><span class="sxs-lookup"><span data-stu-id="2c30f-141">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assembly and creates the stored procedure in the database.</span></span>  
...
```  
 ...
 <span data-ttu-id="2c30f-142">這是 `test.sql`，它會執行函數，藉以測試範例。</span><span class="sxs-lookup"><span data-stu-id="2c30f-142">This is `test.sql`, which tests the sample by executing the functions.</span></span>  
...
```  
 ...
 <span data-ttu-id="2c30f-143">下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 會從資料庫中移除組件和函數。</span><span class="sxs-lookup"><span data-stu-id="2c30f-143">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assembly and functions from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="2c30f-144">另請參閱</span><span class="sxs-lookup"><span data-stu-id="2c30f-144">See Also</span></span>  
 [<span data-ttu-id="2c30f-145">通用語言執行平台 &#40;CLR&#41; 整合的使用案例和範例</span><span class="sxs-lookup"><span data-stu-id="2c30f-145">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
