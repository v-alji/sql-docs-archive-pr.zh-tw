...
ms.locfileid: "87593546"
---
# <a name="calendar-aware-date-and-time-udt-sample"></a><span data-ttu-id="8b558-102">行事曆感知日期和時間 UDT 範例</span><span class="sxs-lookup"><span data-stu-id="8b558-102">Calendar-Aware Date and Time UDT Sample</span></span>
  <span data-ttu-id="8b558-103">將日期儲存為字串可能會產生混淆，因為如果不了解正在使用的日曆系統為何，日期便沒有意義。`CADatetime` 範例會定義兩種使用者定義資料類型 (`CADatetime` 和 `CADate`)，可提供日期和時間的日曆感知處理。</span><span class="sxs-lookup"><span data-stu-id="8b558-103">Storing dates as strings can be confusing because dates are meaningless without understanding what calendar system is being used.The `CADatetime` sample defines two user-defined data types, `CADatetime` and `CADate`, which provide calendar-aware handling of dates and times.</span></span>  
  
## <a name="prerequisites"></a><span data-ttu-id="8b558-104">Prerequisites</span><span class="sxs-lookup"><span data-stu-id="8b558-104">Prerequisites</span></span>  
 <span data-ttu-id="8b558-105">若要建立並執行這個專案，您必須安裝下列軟體：</span><span class="sxs-lookup"><span data-stu-id="8b558-105">To create and run this project the following the following software must be installed:</span></span>  
  
-   [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] <span data-ttu-id="8b558-106">或 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express。</span><span class="sxs-lookup"><span data-stu-id="8b558-106">or [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express.</span></span> <span data-ttu-id="8b558-107">您可以從 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 文件集和範例[網站](https://www.microsoft.com/sql-server/sql-server-editions-express)免費取得 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express</span><span class="sxs-lookup"><span data-stu-id="8b558-107">You can obtain [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express free of charge from the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express Documentation and Samples [Web site](https://www.microsoft.com/sql-server/sql-server-editions-express)</span></span>  
  
-   <span data-ttu-id="8b558-108">您可以從 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 開發人員[網站](https://go.microsoft.com/fwlink/?linkid=62796)取得 AdventureWorks 資料庫</span><span class="sxs-lookup"><span data-stu-id="8b558-108">The AdventureWorks database that is available at the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Developer [Web site](https://go.microsoft.com/fwlink/?linkid=62796)</span></span>  
  
-   <span data-ttu-id="8b558-109">.NET Framework SDK 2.0 或更新版本或是 Microsoft Visual Studio 2005 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="8b558-109">.NET Framework SDK 2.0 or later or Microsoft Visual Studio 2005 or later.</span></span> <span data-ttu-id="8b558-110">您可以免費取得 .NET Framework SDK。</span><span class="sxs-lookup"><span data-stu-id="8b558-110">You can obtain .NET Framework SDK free of charge.</span></span>  
  
-   <span data-ttu-id="8b558-111">此外，您也必須符合下列條件：</span><span class="sxs-lookup"><span data-stu-id="8b558-111">In addition, the following conditions must be met:</span></span>  
  
-   <span data-ttu-id="8b558-112">您所使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體必須啟用 CLR 整合。</span><span class="sxs-lookup"><span data-stu-id="8b558-112">The [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using must have CLR integration enabled.</span></span>  
  
-   <span data-ttu-id="8b558-113">若要啟用 CLR 整合，請執行下列步驟：</span><span class="sxs-lookup"><span data-stu-id="8b558-113">In order to enable CLR integration, perform the following steps:</span></span>  
  
    #### <a name="enabling-clr-integration"></a><span data-ttu-id="8b558-114">啟用 CLR 整合</span><span class="sxs-lookup"><span data-stu-id="8b558-114">Enabling CLR Integration</span></span>  
  
    -   <span data-ttu-id="8b558-115">執行下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 命令：</span><span class="sxs-lookup"><span data-stu-id="8b558-115">Execute the following [!INCLUDE[tsql](../../includes/tsql-md.md)] commands:</span></span>  
    ...
    > [!NOTE]  
    >  <span data-ttu-id="8b558-116">若要啟用 CLR 整合，您必須擁有 `ALTER SETTINGS` 伺服器層級權限，此權限是由系統管理員 (`sysadmin`) 和伺服器管理員 (`serveradmin`) 固定伺服器角色的成員以隱含方式持有。</span><span class="sxs-lookup"><span data-stu-id="8b558-116">To enable CLR, you must have `ALTER SETTINGS` server level permission, which is implicitly held by members of the `sysadmin` and `serveradmin` fixed server roles.</span></span>  
  
-   <span data-ttu-id="8b558-117">AdventureWorks 資料庫必須安裝在您所使用的 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體上。</span><span class="sxs-lookup"><span data-stu-id="8b558-117">The AdventureWorks database must be installed on the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using.</span></span>  
  
-   <span data-ttu-id="8b558-118">如果您不是正在使用之 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體的管理員，則必須讓管理員授與您 **CreateAssembly** 權限來完成安裝。</span><span class="sxs-lookup"><span data-stu-id="8b558-118">If you are not an administrator for the [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance you are using, you must have an administrator grant you **CreateAssembly**  permission to complete the installation.</span></span>  
  
## <a name="building-the-sample"></a><span data-ttu-id="8b558-119">建立範例</span><span class="sxs-lookup"><span data-stu-id="8b558-119">Building the Sample</span></span>  
  
#### <a name="create-and-run-the-sample-by-using-the-following-instructions"></a><span data-ttu-id="8b558-120">使用下列指示來建立並執行範例：</span><span class="sxs-lookup"><span data-stu-id="8b558-120">Create and run the sample by using the following instructions:</span></span>  
  
1.  <span data-ttu-id="8b558-121">開啟 Visual Studio 或 .NET Framework 命令提示字元。</span><span class="sxs-lookup"><span data-stu-id="8b558-121">Open a Visual Studio or .NET Framework command prompt.</span></span>  
  
2.  <span data-ttu-id="8b558-122">必要時，請建立範例的目錄。</span><span class="sxs-lookup"><span data-stu-id="8b558-122">If necessary, create a directory for your sample.</span></span> <span data-ttu-id="8b558-123">在此範例中，我們將使用 C:\MySample。</span><span class="sxs-lookup"><span data-stu-id="8b558-123">For this example, we will use C:\MySample.</span></span>  
  
3.  <span data-ttu-id="8b558-124">在 c:\MySample 中，建立 `CalendarAware.cs` 並將 C# 範例程式碼 (下面) 複製到此檔案中。</span><span class="sxs-lookup"><span data-stu-id="8b558-124">In c:\MySample, create `CalendarAware.cs` and copy the C# sample code (below) into the file.</span></span>  
  
4.  <span data-ttu-id="8b558-125">在 c:\MySample 中，建立 `calendars.txt` 檔案並將範例程式碼複製到此檔案中。</span><span class="sxs-lookup"><span data-stu-id="8b558-125">In c:\MySample, create the file `calendars.txt` and copy the sample code into the file.</span></span>  
  
5.  <span data-ttu-id="8b558-126">在 c:\MySample 中，建立 `calendars.ar-SA.txt` 檔案並將下列程式碼複製到此檔案中：</span><span class="sxs-lookup"><span data-stu-id="8b558-126">In c:\MySample, create the file `calendars.ar-SA.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 1`  
  
6.  <span data-ttu-id="8b558-127">在 c:\MySample 中，建立 `calendars.ja.txt` 檔案並將下列程式碼複製到此檔案中：</span><span class="sxs-lookup"><span data-stu-id="8b558-127">In c:\MySample, create the file `calendars.ja.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 3`  
  
7.  <span data-ttu-id="8b558-128">在 c:\MySample 中，建立 `calendars.``zh-CN.txt` 檔案並將下列程式碼複製到此檔案中：</span><span class="sxs-lookup"><span data-stu-id="8b558-128">In c:\MySample, create the file `calendars.``zh-CN.txt` and copy the following into the file:</span></span>  
  
    -   `; the default calendar for this culture`  
  
    -   `DefaultCalendarID = 8`  
  
8.  <span data-ttu-id="8b558-129">在 c:\MySample 中，建立 `build.com` 檔案並將下列程式碼複製到此檔案中：</span><span class="sxs-lookup"><span data-stu-id="8b558-129">In c:\MySample, create the file `build.com` and copy the following into the file:</span></span>  
...
9. <span data-ttu-id="8b558-130">執行在命令提示字元中建立的檔案，藉以建立附屬組件。</span><span class="sxs-lookup"><span data-stu-id="8b558-130">Build the satellite assembles by executing the file build at the command prompt.</span></span>  
  
10. <span data-ttu-id="8b558-131">在命令列提示字元中執行下列程式碼，藉以編譯範例程式碼：</span><span class="sxs-lookup"><span data-stu-id="8b558-131">Compile the sample code from the command line prompt by executing the following:</span></span>  
...
11. <span data-ttu-id="8b558-132">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安裝程式碼複製到檔案中，並將它儲存成範例目錄中的 `Install.sql`。</span><span class="sxs-lookup"><span data-stu-id="8b558-132">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation code into a file and save it as `Install.sql` in the sample directory.</span></span>  
  
12. <span data-ttu-id="8b558-133">如果此範例安裝在 `C:\MySample\` 以外的目錄中，請依照指示編輯 `Install.sql` 檔案，以指向該位置。</span><span class="sxs-lookup"><span data-stu-id="8b558-133">If the sample is installed in a directory other then `C:\MySample\`, edit the file `Install.sql` as indicated to point to that location.</span></span>  
  
13. <span data-ttu-id="8b558-134">部署組件和預存程序，方法是執行</span><span class="sxs-lookup"><span data-stu-id="8b558-134">Deploy the assembly and stored procedure by executing</span></span>  
  
    -   `sqlcmd -E -I -i install.sql`  
  
14. <span data-ttu-id="8b558-135">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 測試命令腳本複製到檔案中，並將它儲存成 `test.sql` 範例目錄中的。</span><span class="sxs-lookup"><span data-stu-id="8b558-135">Copy [!INCLUDE[tsql](../../includes/tsql-md.md)] test command script into a file and save it as `test.sql` in the sample directory.</span></span>  
  
15. <span data-ttu-id="8b558-136">使用下列命令來執行測試指令碼</span><span class="sxs-lookup"><span data-stu-id="8b558-136">Execute the test script with the following command</span></span>  
  
    -   `sqlcmd -E -I -i test.sql`  
  
16. <span data-ttu-id="8b558-137">將 [!INCLUDE[tsql](../../includes/tsql-md.md)] 清除指令碼複製到檔案中，並將它儲存成範例目錄中的 `cleanup.sql`。</span><span class="sxs-lookup"><span data-stu-id="8b558-137">Copy the [!INCLUDE[tsql](../../includes/tsql-md.md)] cleanup script into a file and save it as `cleanup.sql` in the sample directory.</span></span>  
  
17. <span data-ttu-id="8b558-138">使用下列命令來執行指令碼</span><span class="sxs-lookup"><span data-stu-id="8b558-138">Execute the script with the  following command</span></span>  
  
    -   `sqlcmd -E -I -i cleanup.sql`  
  
## <a name="sample-code"></a><span data-ttu-id="8b558-139">範例程式碼</span><span class="sxs-lookup"><span data-stu-id="8b558-139">Sample Code</span></span>  
 <span data-ttu-id="8b558-140">下面是此範例的程式碼清單。</span><span class="sxs-lookup"><span data-stu-id="8b558-140">The following are the code listings for this sample.</span></span>  
  
 <span data-ttu-id="8b558-141">C#</span><span class="sxs-lookup"><span data-stu-id="8b558-141">C#</span></span>  
...
```  
 ...
 <span data-ttu-id="8b558-142">這是定義日曆所需的 `calendars.txt` 檔案。</span><span class="sxs-lookup"><span data-stu-id="8b558-142">This is the file `calendars.txt` needed for defining the calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="8b558-143">這是 [!INCLUDE[tsql](../../includes/tsql-md.md)] 安裝指令碼 (`Install.sql`)，它會部署組件並在資料庫中建立轉換函數。此外，它也會建立範例資料的資料表。</span><span class="sxs-lookup"><span data-stu-id="8b558-143">This is the [!INCLUDE[tsql](../../includes/tsql-md.md)] installation script (`Install.sql`), which deploys the assemblies and creates the conversion functions within the database; it also crates a table of sample data.</span></span>  
...
```  
 ...
 <span data-ttu-id="8b558-144">這是 `test.sql`，它會在各種日曆上執行函數，藉以測試範例。</span><span class="sxs-lookup"><span data-stu-id="8b558-144">This is `test.sql`, which tests the sample by executing the functions on various calendars.</span></span>  
...
```  
 ...
 <span data-ttu-id="8b558-145">下列 [!INCLUDE[tsql](../../includes/tsql-md.md)] 會從資料庫中移除組件、函數和資料表。</span><span class="sxs-lookup"><span data-stu-id="8b558-145">The following [!INCLUDE[tsql](../../includes/tsql-md.md)] removes the assemblies, functions and table from the database.</span></span>  
...
## <a name="see-also"></a><span data-ttu-id="8b558-146">另請參閱</span><span class="sxs-lookup"><span data-stu-id="8b558-146">See Also</span></span>  
 [<span data-ttu-id="8b558-147">通用語言執行平台 &#40;CLR&#41; 整合的使用案例和範例</span><span class="sxs-lookup"><span data-stu-id="8b558-147">Usage Scenarios and Examples for Common Language Runtime &#40;CLR&#41; Integration</span></span>](../../../2014/database-engine/dev-guide/usage-scenarios-and-examples-for-common-language-runtime-clr-integration.md)  
