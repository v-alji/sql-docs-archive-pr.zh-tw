...
7.   執行個體不得為 SQL Server 容錯移轉叢集的一部分。 使用 SharePoint 產品的高可用性功能。 例如，Excel Services 會管理 PowerPivot for SharePoint 伺服器的負載平衡。 如需詳細資訊，請參閱[ (SharePoint Server 2013)  (管理 Excel Services 資料模型設定](https://technet.microsoft.com/library/jj219780.aspx) https://technet.microsoft.com/library/jj219780.aspx) 。
...
9. 您可選擇性地在安裝中加入一項 Database Engine 的執行個體。 如果您要設定新的伺服器陣列，且需要資料庫伺服器來執行伺服器陣列的設定和內容資料庫，您可能會這麼做。 若您加入 Database Engine，將會安裝為 PowerPivot 具名執行個體。 當您需要指定與此實例的連接時 (例如，如果您使用該嚮導來設定伺服器陣列) ，請以下列格式輸入資料庫名稱： <`servername`> \powerpivot。
