...
|特徵|[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 中引入的變更|  
|-------------|-----------------------------------------------------|  
...
|[!INCLUDE[ssSB](../../includes/sssb-md.md)]|[!INCLUDE[ssSB](../../includes/sssb-md.md)]保留不能保留在記憶體中的現有對話內容 (大約每個對話方塊) 1 KB 時，會明確使用**tempdb** 。<br /><br /> [!INCLUDE[ssSB](../../includes/sssb-md.md)]會透過在查詢執行內容中的物件快取，隱含使用**tempdb** 。 例如，用於計時器事件和背景傳遞交談的工作資料表。<br /><br /> DBMail、事件通知和查詢通知等功能會隱含使用 [!INCLUDE[ssSB](../../includes/sssb-md.md)]。|  
|大型物件 (LOB) 資料類型<br /><br /> LOB 變數與參數|資料類型 `varchar(max)` 、 `nvarchar(max)` 、 **Varbinary (最大) 文字**、 `ntext` `image,` 和 `xml` 是大型物件類型。<br /><br /> 在資料庫上啟用以資料列版本設定為基礎的交易隔離等級，並對大型物件進行修改時，會將已變更的 LOB 片段複製到**tempdb**中的版本存放區。<br /><br /> 定義為大型物件資料類型的參數會儲存在**tempdb**中。|  
