 ...
 **類別**  
 顯示運算式的類別目錄。 選擇類別目錄會為運算式的建立奠立內容，並會變更 [項目] 窗格中的有效值清單。 例如，針對文字方塊值的運算式，展開 [一般函數]，然後選取 [彙總函式 `Avg` ]，以顯示、 `Count` 和 [**專案**] 窗格中的其他函數。  
