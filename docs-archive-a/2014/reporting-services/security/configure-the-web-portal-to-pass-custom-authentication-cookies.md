...
## <a name="modifying-the-rsreportserverconfig-file"></a>修改 RSReportServer.Config 檔  
 您可以藉由將 <> 專案新增 `PassThroughCookies` 至 RSReportServer.config 檔案中的報表管理員設定，讓報表管理員將額外的 cookie 傳送至報表伺服器。 當單一登入驗證方案不僅需要報表伺服器驗證 Cookie，同時也需要協力廠商驗證系統提供的 Cookie 時，傳送其他 Cookie 這項功能就顯得非常有用。  
