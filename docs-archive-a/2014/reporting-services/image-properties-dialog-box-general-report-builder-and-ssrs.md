 ...
 **工具提示**  
 輸入文字或評估為工具提示的運算式。 按一下運算式 (*fx*) ] 按鈕，即可編輯運算式。 使用者在 HTML 報表中將指標暫停在項目上時，[工具提示]**** 會顯示。  
 ...
 **使用此影像**  
 ...
 如果您選取 [外部]**** 選項，輸入影像的 URL。 對於發行到設定為原生模式之報表伺服器的報表，請使用完整或相對路徑， 例如，HTTP:// \<servername> /images/image1.jpg。 對於發行到設定為 SharePoint 整合模式之報表伺服器的報表，請使用完整 URL， 例如，HTTP:// \<*SharePointservername*> / \<*site*> /Documents/images/image1.jpg。  
