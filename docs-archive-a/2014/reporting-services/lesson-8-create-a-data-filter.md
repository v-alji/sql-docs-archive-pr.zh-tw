...
4.  在 [**運算式**] 欄位 `ProductID` 的下拉式清單中，按一下。 這是要套用篩選的資料行。  
  
5.  **=** 在 [**運算子**] 下拉式清單中，按一下等於 () ] 運算子。  
  
6.  按一下 [**值**] 欄位旁的 [運算式] 按鈕，按一下 [**類別目錄**] 區域中的 [**參數**]，然後 `productid` 在 [**值**] 區域中按兩下。 [設定運算式對象: 值]  欄位現在應該包含類似 **=Parameters!productid.Value** 的運算式。  
