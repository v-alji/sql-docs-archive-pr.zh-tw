...
2.  在方塊中 `Name` ，輸入： **ListDataSource**

3.  按一下 **[使用內嵌於報表中的連接]**。

4.  確認連線類型為 [Microsoft SQL Server]，然後在 [**連接字串**] 方塊中輸入： **Data Source \<servername> =**

     \<servername>例如，Report001) 會指定已安裝 SQL Server 資料庫引擎實例的電腦。 由於報表資料不是擷取自 SQL Server 資料庫，您不必加上資料庫的名稱。 指定之伺服器上的預設資料庫將用來剖析查詢。
...
4.  在中 `Name` ，將預設名稱取代為**為 salesinformationbyterritory**。
...
3.  在中 `Name` ，將預設名稱取代為**為 salesinformationbyterritory**。

4.  按一下 [檔案] 。

##  <a name="8-optional-add-a-line-to-separate-areas-of-the-report"></a><a name="Line"></a>8. (選擇性) 加入線條以區隔報表的各區域
 加入線條以區隔報表的編輯區和詳細資料區。
...
##  <a name="9-optional-add-summary-data-visualization"></a><a name="Visualization"></a>9. (選擇性) 新增摘要資料視覺效果
