...
13. 在 [**值**] 清單中，以滑鼠右鍵按一下 `CountryRegionID` 並按一下 [ **Sum** ] 選項。  
...
##  <a name="6-count-days-since-last-purchase"></a><a name="Count"></a>6. 計算自上次購買後的天數  
 新增資料行，然後使用**Now**函數或 `ExecutionTime` 內建全域變數，計算自個人上次購買後的當日天數。  
...
##  <a name="7-use-an-indicator-to-show-sales-comparison"></a><a name="Indicator"></a>7. 使用指標顯示銷售比較  
 加入新的資料行，並使用指標來顯示使用者的年初迄今 (YTD) 購買高於或低於平均 YTD 購買。 **Round** 函數會移除值的小數。  
...
##  <a name="optional-format-date-column"></a><a name="DateFormat"></a> (選擇性) 格式日期資料行  
 格式化包含日期的 [上次購買]**** 資料行。  
