...
# <a name="add-change-or-delete-a-map-or-map-layer-report-builder-and-ssrs"></a>加入、變更或刪除地圖或地圖圖層 (報表產生器及 SSRS)
 ...
 地圖圖層是以它們顯示在 [地圖] 窗格中的順序，由下而上繪製。 在上圖中，圖格圖層會最先繪製，而多邊形圖層則是最後繪製。 稍後繪製的圖層可能會隱藏圖層上先前繪製的地圖元素。 您可以使用 [地圖] 窗格工具列上的方向鍵來變更圖層的順序。 若要顯示或隱藏圖層，請切換可見性圖示。 您可以在 `Visibility` [**圖層資料**屬性] 對話方塊的頁面上，變更圖層的透明度。  
