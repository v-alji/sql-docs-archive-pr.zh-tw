...
|完成：已 \<*number*> 處理 \<*number*> 總計; 個 \<*number*> 錯誤。|顯示資料驅動訂閱執行的狀態；此訊息來自排程與傳遞處理器。|  
|\<*number*>得到|排程與傳遞處理器已成功傳遞或已不再嘗試傳遞的通知數目。 當資料驅動傳遞完成時，已處理的通知數目應該和產生的通知總數相等。|  
|\<*number*>總數|訂閱最後一次傳遞所產生的通知總數。|  
|\<*number*>糾錯|排程與傳遞處理器無法傳遞或已不再嘗試傳遞的通知數目。|  
...
|無法連接到目的資料夾， \<path> 。 請確認目的資料夾或檔案共用存在。|指出找不到所指定的資料夾；此訊息來自檔案共用傳遞延伸模組。|  
|\<filename>無法寫入檔案 \<path> 。 正在嘗試重試。|指出無法以較新版本進行檔案更新；此訊息來自檔案共用傳遞延伸模組。|  
|無法寫入檔案 \<filename> ：\<message>|指出未成功傳遞到檔案共用位置；此訊息來自檔案共用傳遞延伸模組。|  
