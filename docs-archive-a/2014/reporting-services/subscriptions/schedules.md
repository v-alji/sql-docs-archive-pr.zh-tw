...
##  <a name="server-dependencies"></a><a name="bkmk_serverdependencies"></a> 伺服器相依性  
 排程與傳遞處理器需要啟動報表伺服器服務與 SQL Server Agent。 排程和傳遞處理功能必須透過 `ScheduleEventsAndReportDeliveryEnabled` 以原則為基礎的管理中的**Reporting Services Facet 介面區**設定的屬性來啟用。 SQL Server Agent 與報表伺服器服務都必須執行，排程的作業才會發生。  
