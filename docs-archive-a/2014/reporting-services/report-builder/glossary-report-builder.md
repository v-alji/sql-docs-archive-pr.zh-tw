...
|aggregation|包含預先計算之 Cube 資料的資料表或結構。|  
...
|複雜運算式 (complex expression)|運算式無法以單一內建欄位參考表示，而且在設計介面上會顯示為 <\<Expr>>。|  
...
|gauge|量測計是資料區，會將資料呈現成範圍，其中包含指向範圍內特定值的指標。|  
...
|整數|包括 bigint、int、smallint 和 tinyint 等資料類型的數值資料類型類別目錄。|  
...
|延遲|從一部伺服器完成資料變更，到變更出現在另一部伺服器所經歷的時間量。|  
...
|object|其中一種資料庫元件，例如資料表、索引或預存程序。|  
...
|指令碼|用於執行作業的 Transact-SQL 陳述式集合。|  
