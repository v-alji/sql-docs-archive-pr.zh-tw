...
-   [一般設定 ( # A0) ](#bkmk_generalconfiguration)  
  
-   [URLReservations ( # A0 file) ](#bkmk_URLReservations)  
  
-   [驗證 ( # A0 file) ](#bkmk_Authentication)  
  
-   [服務 ( # A0 檔案) ](#bkmk_service)  
  
-   [傳遞 UI 延伸模組 (s) 一般設定](#bkmk_ui)  
  
-   [擴充功能 ( # A0 file) 原生模式](#bkmk_extensions)  
  
    -   [傳遞延伸模組 (s) 一般設定](#bkmk_extensionsgeneral)  
...
|**LogonMethod**|這個值會指定的登入類型 `RSWindowsBasic` 。 如果您指定 `RSWindowsBasic` ，則需要此值。 有效值為 2 或 3，其中每個值代表下列項目：<br /><br /> `2` = 網路登入，用於驗證純文字密碼的高效能伺服器。<br /><br /> `3` = 純文字登入，可將登入認證保存在隨著每個 HTTP 要求傳送的驗證封裝中，以便在連接至網路中的其他伺服器時，允許伺服器模擬使用者。<br /><br /> 注意:[!INCLUDE[ssRSCurrent](../../includes/ssrscurrent-md.md)] 中不支援值 0 (用於互動式登入) 和 1 (用於批次登入)。|N|  
...
|**SendEmailToUserAlias**|此值使用 **DefaultHostName**。<br /><br /> 當**SendEmailToUserAlias**設定為時 `True` ，定義個別訂閱的使用者會自動指定為報表的收件者。 **[收件者]** 欄位是隱藏的。 如果這個值為 `False` ，則會顯示 [**到**] 欄位。 如果您希望充分控制報表散發，請將此值設定為 `True`。 有效值如下：<br /><br /> `True`= 使用建立訂閱之使用者的電子郵件地址。 這是預設值。<br /><br /> `False`=可以指定任何電子郵件地址。|  
...
-   [轉譯延伸模組的裝置資訊設定 &#40;Reporting Services&#41;](../device-information-settings-for-rendering-extensions-reporting-services.md)  
 ...
 如需底下子項目屬性的詳細資訊 **\<Extension>** **\<Render>** ，請參閱下列各項：  
