...
##  <a name="to-configure-sql-server-management-studio-ssms-for-local-report-server-administration"></a><a name="bkmk_configure_ssms"></a>若要設定 SQL Server Management Studio (SSMS) 進行本機報表伺服器管理  
...
##  <a name="to-configure-sql-server-data-tools-bi-ssdt-to-publish-to-a-local-report-server"></a><a name="bkmk_configure_ssdt"></a>若要設定 SQL Server Data Tools BI (SSDT) 發行至本機報表伺服器  
...
1.  在 [開始] 畫面上，輸入 `sql server` ，然後以滑鼠右鍵按一下**Visual Studio 的 [SQL Server Data Tools**]。 按一下 **[以系統管理員身分執行]**。  
