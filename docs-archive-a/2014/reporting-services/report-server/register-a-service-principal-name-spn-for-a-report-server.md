...
-   [ (spn 的服務主體名稱) SetSPN 語法 ( # A0) ](https://social.technet.microsoft.com/wiki/contents/articles/717.service-principal-names-spns-setspn-syntax-setspn-exe.aspx) (https://social.technet.microsoft.com/wiki/contents/articles/717.service-principal-names-spns-setspn-syntax-setspn-exe.aspx) 。  
...
```  
 ...
 您為、和指定的值，會 \<*computername*> \<*domainname*> \<*port*> 識別主控報表伺服器之電腦的唯一網路位址。 這個值可以是本機主機名稱或完整網域名稱 (FQDN)。 如果您只有一個網域，而且使用通訊埠80，您可以在 \<*domainname*> \<*port*> 命令列中省略和。 \<*domain-user-account*>這是用來執行報表伺服器服務以及必須註冊 SPN 的使用者帳戶。  
