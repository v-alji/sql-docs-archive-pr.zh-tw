...
|210046|成員不可以是 [根] 的同層級。|當**RelationshipType**  =  **2** (兄弟) ，而**ParentCode**或**ChildCode**是**Root**時，就會發生這種情況。 成員不得與根節點位於相同層級；它們僅能為子系。|關聯性|  
|210047|成員不可以是 [未使用] 的同層級。|當**RelationshipType**  =  **2** (兄弟) ，而**ParentCode**或**ChildCode**未**使用**時，就會發生這種情況。 成員只能是 [未使用] 節點的子項。|關聯性|  
