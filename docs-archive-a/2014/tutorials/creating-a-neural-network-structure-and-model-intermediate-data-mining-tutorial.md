...
11. 選取 [索引**鍵**]、[**輸入**] 和核取方塊，如下 `Predict` 表所示：  
...
15. 針對 [**採礦模型名稱**]，輸入 `Call Center Default NN` ，然後按一下 **[完成]**。  
...
-   **透過同等區域進行分類收納**  
 ...
 或者，您可以新增個別的衍生資料行，將服務等級分類成預先定義的目標範圍，例如**最佳** (ServiceGrade \<= 0.05), **Acceptable** (0.10 > ServiceGrade > 0.05) ，以及不**佳**的 (ServiceGrade >= 0.10) 。  
...
8.  將屬性 `Content` 的值從 `Continuous` 變更為 `Discretized`。  
    ...
    |`DiscretizationBucketCount`|沒有任何值|4|  
...
4.  在 [**新增採礦模型**] 對話方塊的 [**模型名稱**] 中，輸入 `Call Center Binned NN` 。 在 [**演算法名稱**] 下拉式清單中，選取 [ **Microsoft 類神經網路**]。  
...
5.  按一下 [**採礦模型**] 索引標籤中的任何位置。  
     ...
     方格會更新，以顯示資料行使用方式旁邊的新暫存資料行別名 `ServiceGrade` 。 包含採礦結構及兩個採礦模型的方格應該如下所示：  
    ...
    |訂單|輸入|輸入|  
    |ServceGrade Binned|忽略|Predict (ServiceGrade)|  
...
2.  在 [HOLDOUT_SEED] 參數的資料列中，按一下 [**值**] 底下的空白資料格，然後輸入 `1` 。 按一下 [確定]  。 針對與此結構有關的每一個模型重複這個步驟。  
