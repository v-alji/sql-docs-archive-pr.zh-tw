...
2.  在 [資料來源 View wizard] 中，選取 [!INCLUDE[ssAWDWsp](../includes/ssawdwsp-md.md)] 做為資料來源，然後按 **[下一步]**。  
...
6.  在 [**建立命名查詢**] 對話方塊中，針對 [**名稱**] 輸入 `Shifts for Call Center` 。  
...
9. 以滑鼠右鍵按一下索引標籤，將** (設計) ]，** 然後按一下 [**儲存**] 以儲存新的資料來源視圖定義。  
...
3.  從 [**採礦模型**] 功能表中，清除 [**單一查詢**] 選項。 此時會出現一個警告，告知您將會失去單一查詢輸入。 按一下 [確定]  。  
     ...
     [**單一查詢輸入**] 對話方塊會取代為 [**選取輸入資料表 (s) ** ] 對話方塊。  
  
4.  按一下 [選取案例資料表] ****。  
  
5.  在 [**選取資料表**] 對話方塊中，從資料來源清單中 selectShifts。 在 [**資料表/視圖名稱**] 清單中，選取 [撥接中心的位移] (它可能會自動選取) ]，然後按一下 **[確定]。**  
