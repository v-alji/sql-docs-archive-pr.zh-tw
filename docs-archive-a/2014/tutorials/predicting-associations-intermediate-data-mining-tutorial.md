...
4.  在方格中，按一下 [**來源**] 底下的空白資料格，然後選取 [**預測函數]。** 在 [**欄位**] 底下的資料格中，選取 `PredictAssociation` 。  
  
     您也可以使用**predict**函數來預測關聯。 如果您這樣做，請務必選擇採用資料表資料行做為引數的**Predict**函數版本。  
  
5.  在 [**採礦模型**] 窗格中，選取 [嵌套] 資料表 `vAssocSeqLineItems` ，並將它拖曳到方格中，指向函數的 [**準則/引數**] 方塊 `PredictAssociation` 。  
  
     拖曳資料表和資料行名稱可讓您建立複雜的陳述式，而不會產生語法錯誤。 不過，它會取代資料格的目前內容，其中包括函數的其他選擇性引數 `PredictAssociation` 。 若要檢視其他引數，您可以暫時將此函數的第二個執行個體加入至方格內以供參考。  
...
5.  在 [**採礦模型**] 窗格中，選取 [嵌套] 資料表 `vAssocSeqLineItems` ，並將它拖曳到方格中，指向函數的 [**準則/引數**] 方塊 `PredictAssociation` 。 在 `,3` 嵌套的資料表名稱後面輸入，如同上一個程式所示。  
  
6.  在 [**單一查詢輸入**] 對話方塊中，按一下 [ **VAssoc Seq Line Items**] 旁的 [**值**] 方塊，然後按一下 [ ** ( ... ) ** ] 按鈕。  
  
7.  在 [**嵌套資料表輸入**] 對話方塊中，選取 `Touring Tire` [索引鍵資料**行**] 窗格中的，然後按一下 [**加入**]。  
...
3.  在 [索引**鍵資料行**] 窗格中，選取 [] `Touring Tire` ，然後按一下 [**新增**]。  
  
4.  在方格中，按一下 [**來源**] 底下的空白資料格，然後選取 [**預測函數]。** 在 [**欄位**] 底下的資料格中，選取 `PredictAssociation` 。  
  
5.  在 [**採礦模型**] 窗格中，選取 [嵌套] 資料表 `vAssocSeqLineItems` ，並將它拖曳到方格中，指向函數的 [**準則/引數**] 方塊 `PredictAssociation` 。 在 `,3` 嵌套的資料表名稱後面輸入，如同上一個程式所示。  
  
6.  在 [**嵌套資料表輸入**] 對話方塊中，選取 `Touring Tire Tube` [索引鍵資料**行**] 窗格中的，然後按一下 [**加入**]。  
  
7.  在方格中，于函數的資料列中 `PredictAssociation` ，按一下 [**準則/引數**] 方塊，然後變更引數以加入引數 INCLUDE_STATISTICS。  
...
11. 將新的資料列加入方格中。 針對 [**來源**] 選取 [**預測函數**]，然後針對 [**欄位**] 選取 [] `PredictAssociation` 。  
  
12. 將 [vAssocSeqLineItems] 拖曳到資料列的 [**準則/引數**] 方塊中 `PredictAssociation` 。 按一下 [**準則/引數**] 方塊的結尾，然後輸入下列文字：`INCLUDE_STATISTICS,3`  
