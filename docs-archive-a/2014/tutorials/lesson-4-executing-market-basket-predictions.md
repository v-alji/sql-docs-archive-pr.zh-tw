...
# <a name="lesson-4-executing-market-basket-predictions"></a>第 4 課：執行購物籃預測
 ...
 語句的**SELECT FROM \<model> 預測聯結**形式 `SELECT` 包含三個部分：  
...
## <a name="singleton-prediction-join-statement"></a>單一 PREDICTION JOIN 陳述式  
 第一個步驟是建立單一查詢，方法是使用**SELECT FROM \<model> 預測聯結**語法，並提供一組值做為輸入。 以下是單一陳述式的一般範例：  
...
1.  在**物件總管**中，以滑鼠右鍵按一下的實例 [!INCLUDE[ssASnoversion](../includes/ssasnoversion-md.md)] ，指向 [追加**查詢**]，然後按一下 [ **DMX** ] 以開啟 [查詢編輯器]。  
...
7.  在 [**另存**新檔] 對話方塊中，流覽至適當的資料夾，並將檔案命名為 `Association Prediction.dmx` 。  
...
7.  在 [**另存**新檔] 對話方塊中，流覽至適當的資料夾，並將檔案命名為 `Modified Association Prediction.dmx` 。  
