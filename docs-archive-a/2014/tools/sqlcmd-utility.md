...
# <a name="sqlcmd-utility"></a>sqlcmd 公用程式
  `sqlcmd`公用程式可讓您在 [!INCLUDE[tsql](../includes/tsql-md.md)] 命令提示字元處、在 SQLCMD 模式的 [**查詢編輯器**] 中、在 Windows 腳本檔案中，或是在 Agent 作業的作業系統 ( # A0) 作業步驟中，輸入語句、系統程式和腳本檔案 [!INCLUDE[ssNoVersion](../includes/ssnoversion-md.md)] 。 這個公用程式利用 ODBC 執行 [!INCLUDE[tsql](../includes/tsql-md.md)] 批次。  
...
3.  在執行之前，命令 shell (**set** X = Y) 設定于命令提示字元 `sqlcmd` 。  
 ...
 **執行控制命令**  
 ...
 `sqlcmd`公用程式會將** ( # B1**的括弧之間的所有內容傳送至伺服器。 如果系統預存程序選取某一組，傳回某個值，此時只會傳回選取的項目。 括號中沒有任何內容的 EXIT **()** 陳述式，會執行批次中在它前面的任何內容，然後結束作業，不傳回任何值。  
