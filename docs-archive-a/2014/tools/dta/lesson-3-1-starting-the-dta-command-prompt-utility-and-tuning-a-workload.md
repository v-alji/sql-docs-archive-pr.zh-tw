...
# <a name="starting-the-dta-command-prompt-utility-and-tuning-a-workload"></a>啟動 dta 命令提示字元公用程式和微調工作負載
   這項工作會帶您逐步啟動 **dta** 公用程式、檢視它的說明，再從命令提示字元之下，使用它來微調工作負載。 它會使用您為 Database Engine Tuning Advisor 圖形化使用者介面所建立的工作負載 Myscript.sql， (GUI) 練習[微調工作負載](lesson-1-1-tuning-a-workload.md)。  
