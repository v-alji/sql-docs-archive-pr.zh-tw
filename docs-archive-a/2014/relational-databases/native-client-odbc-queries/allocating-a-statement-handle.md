...
# <a name="allocating-a-statement-handle"></a>配置陳述式控制代碼
 ...
 使用數個 ODBC 陳述式與連接選項時請小心。 呼叫[SQLSetConnectAttr](../native-client-odbc-api/sqlsetconnectattr.md)並將*fOption*設為 SQL_ATTR_LOGIN_TIMEOUT 可控制應用程式在等候建立連接時等待連線嘗試超時的時間 (0 指定無限等候) 。 回應時間緩慢的網站可以將這個值設高一點，以確認連接有充足的時間可以完成。 不過，如果驅動程式無法連接，間隔應該一律夠低，才能在合理的時間內回應使用者。  
