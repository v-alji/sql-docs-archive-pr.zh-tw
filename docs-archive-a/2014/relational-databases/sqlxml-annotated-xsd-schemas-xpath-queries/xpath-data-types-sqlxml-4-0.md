...
## <a name="xpath-conversions"></a>XPath 轉換  
 當您使用 XPath 查詢 (例如，`OrderDetail[@UnitPrice > "10.0"]`) 時，隱含和明確的資料類型轉換可能會以明顯的方式變更查詢的意義。 因此，了解如何實作 XPath 資料類型相當重要。 您可以在位於的 W3C 網站，找到 XPath 語言規格、XML 路徑語言 (XPath) 1.0 版 W3C 建議的建議 8 1999 http://www.w3.org/TR/1999/PR-xpath-19991008.html 。  
...
## <a name="mapping-xdr-data-types-to-xpath-data-types"></a>將 XDR 資料類型對應到 XPath 資料類型  
 節點的 XPath 資料類型衍生自架構中的 XDR 資料類型，如下表所示 (節點的「**員工 id** 」用於說明目的) 。  
...
### <a name="a-convert-a-data-type-in-an-xpath-query"></a>A. 在 XPath 查詢中轉換資料類型  
 ...
 因為「**員工 id** 」是 `id` (`idref` 、、、等其中一個， `idrefs` `nmtoken` `nmtokens` 所以在 XSD 架構中) 資料類型值**EmployeeID** ，因此會 `string` 使用先前所述的轉換規則，將「員工」轉換為 XPath 資料類型。  
