...
-   在命令提示字元中輸入 `sqlps`，然後按下 ENTER。 輸入 `cd xevent`，然後按 ENTER 鍵。 從該處，您可以使用**cd**和 `dir` 命令 (或**設定位置**和**get-childitem** Cmdlet，) 流覽至伺服器名稱和實例名稱。  
...
-   腳本必須從 PS SQLSERVER： \\> 提示字元中執行， (可以 `sqlps` 在命令提示字元中輸入) 。  
