...
# <a name="specifying-the-sqlinverse-attribute-on-sqlrelationship-sqlxml-40"></a>針對 sql:relationship 指定 sql:inverse 屬性 (SQLXML 4.0)
 ...
 如果您有 XSD 結構描述，而且其父子式關聯性是以對應資料庫資料行之間主索引鍵/外部索引鍵關聯性的反向順序所指定，則插入或刪除 Updategram 作業將會由於主索引鍵/外部索引鍵違規而失敗。 在這種情況下， `sql:inverse` 屬性會在專案中指定 (`sql:inverse="true"`) **\<sql:relationship>** ，而 updategram 邏輯則會反轉其在架構中指定之父子式關聯性的轉譯。  
