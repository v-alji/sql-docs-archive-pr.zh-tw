...
# <a name="creating-cdata-sections-using-sqluse-cdata-sqlxml-40"></a>使用 sql:use-cdata 建立 CDATA 區段 (SQLXML 4.0)
 ...
 Microsoft 中的資料庫 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 有時可以包含 XML 剖析器視為標記字元的字元; 例如，角括弧 (\< and >) 、小於或等於符號 ( # B0 =) ，而 & ( # A1) 會視為標記字元。 但是，您可以將這類型的特殊字元包裝在 CDATA 區段內，以免被視為標記字元。 XML 剖析器會將 CDATA 區段內的文字視為純文字。  
...
### <a name="a-specifying-sqluse-cdata-on-an-element"></a>A. 在元素上指定 sql:use-cdata  
 在下列架構中， `sql:use-cdata` 會將專案中的設為 1 (True) **\<AddressLine1>** **\<Address>** 。 因此，資料會在 CDATA 區段內傳回。  
