...
## <a name="traditional-login-and-user-model"></a>傳統的登入和使用者模型  
 ...
 最重要的原則是登入 (在 master 資料庫中) 和使用者 (在使用者資料庫中) 必須存在，且彼此相關。 這表示使用者資料庫的連接在登入 master 資料庫時有相依性，而且這會限制資料庫移到不同的裝載 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 或 [!INCLUDE[ssSDSfull](../../includes/sssdsfull-md.md)] 伺服器的能力。 而且，如果因為任何原因而無法使用 master 資料庫的連接 (例如，容錯移轉正在進行中) ，則整體連線時間將會增加，或連接可能會超時。因此，這可能會減少連接的擴充性。  
