...
|DATABASE_PRINCIPAL_IMPERSONATION_GROUP|當資料庫範圍內有模擬作業 (例如 EXECUTE AS \<principal> 或 SETPRINCIPAL) 時，就會引發這個事件。 這個事件是針對任何資料庫中進行的模擬作業而引發。 等於＜ [Audit Database Principal Impersonation Event Class](../../event-classes/audit-database-principal-impersonation-event-class.md)＞。|  
...
|SERVER_PRINCIPAL_IMPERSONATION_GROUP|當伺服器範圍內有模擬情況 (例如 EXECUTE AS \<login>) 時，就會引發這個事件。 等於＜ [Audit Server Principal Impersonation Event Class](../../event-classes/audit-server-principal-impersonation-event-class.md)＞。|  
