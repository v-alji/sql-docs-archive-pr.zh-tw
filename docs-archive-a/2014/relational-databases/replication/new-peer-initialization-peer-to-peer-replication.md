...
# <a name="new-peer-initialization-peer-to-peer-replication"></a>新的對等 (Peer) 初始化 (點對點複寫)
  您可以使用 **[新的對等 (Peer) 初始化]** 頁面來指定對等 (Peer) 資料庫的初始化方式 您必須先初始化 (對等，才能完成此 wizard。 ) 對等是以手動方式初始化，或使用由異動複寫所提供的「**使用初始化**」的備份功能。  (點對點異動複寫不支援使用快照集初始化對等。 ) 如果必須使用不同的方法來初始化不同的對等，您必須執行嚮導多次，以個別新增對等。  
...
-   如果您已還原備份，且第一個發行集資料庫在執行備份之後已變更資料，請選取第二個選項。 如此一來，複寫就必須傳遞來自第一個發行集資料庫中、未包含在備份內的變更。 此選項對應至訂閱屬性 **sync_type** 的 **initialize with backup**值。  
    ...
    |[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]|[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]|無|啟用|  
    |[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]|[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]|無|啟用|  
    ...
    |[!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)]|[!INCLUDE[ssVersion2005](../../includes/ssversion2005-md.md)]|無|啟用|  
