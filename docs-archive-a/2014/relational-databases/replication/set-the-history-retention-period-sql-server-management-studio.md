...
# <a name="set-the-history-retention-period-sql-server-management-studio"></a>設定記錄保留週期 (SQL Server Management Studio)
  在 [散發資料庫屬性 - \<DistributionDatabase>] 對話方塊的 [一般] 頁面上指定記錄保留期間。 此設定會控制複寫代理程式記錄的儲存時間。 此頁面會在 [散發者屬性 - \<Distributor>] 對話方塊的 [一般] 頁面上提供。 如需存取此對話方塊的詳細資訊，請參閱[檢視及修改散發者和發行者屬性](view-and-modify-distributor-and-publisher-properties.md)。  
