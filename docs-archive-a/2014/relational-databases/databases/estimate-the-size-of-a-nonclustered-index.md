...
5.  計算索引資料列的大小：  
     ...
     ***Leaf_Row_Size***   = ***Fixed_Leaf_Size***  + ***Variable_Leaf_Size***  + ***Leaf_Null_Bitmap*** + 1 (索引資料列的資料列行首的額外負荷) + 6 (子頁面識別碼指標)   
...
1.  計算索引中的非分葉層級數目：  
     ...
     ***非分葉層級***= 1 + log Index_Rows_Per_Page (***Num_Leaf_Pages***  /  ***Index_Rows_Per_Page***)   
  
     將這個值無條件向上進位到最近的整數。 此值不包含非叢集索引的分葉層級。  
  
2.  計算索引中的非分葉頁面數目：  
  
     ***Num_Index_Pages*** = ∑層級 (***Num_Leaf_Pages/Index_Rows_Per_Page***<sup>層級</sup>) 其中 1 <= 層級 <=***層***級  
