...
## <a name="binding-parameters-for-sql-character-types"></a>SQL 字元類型的繫結參數  
 如果傳入的 SQL 資料類型是字元類型，則*ColumnSize*是以字元為單位的大小， (不是位元組) 。 如果資料字串的長度（以位元組為單位）大於8000，則*ColumnSize*應該設定為 `SQL_SS_LENGTH_UNLIMITED` ，表示 SQL 類型的大小沒有任何限制。  
...
## <a name="sqlbindparameter-support-for-enhanced-date-and-time-features"></a>SQLBindParameter 對增強型日期和時間功能的支援  
 日期/時間類型的參數值會依照[從 C 轉換成 SQL](../native-client-odbc-date-time/datetime-data-type-conversions-from-c-to-sql.md)中所述的方式進行轉換。 請注意，和類型的參數 `time` `datetimeoffset` 必須*ValueType*指定為 `SQL_C_DEFAULT` 或， `SQL_C_BINARY` 如果其對應的結構 (`SQL_SS_TIME2_STRUCT` 並 `SQL_SS_TIMESTAMPOFFSET_STRUCT` 使用) 。  
