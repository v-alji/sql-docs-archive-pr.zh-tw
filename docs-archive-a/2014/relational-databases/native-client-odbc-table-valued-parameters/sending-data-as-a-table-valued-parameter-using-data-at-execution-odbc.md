...
# <a name="sending-data-as-a-table-valued-parameter-using-data-at-execution-odbc"></a>使用資料執行中 (ODBC) 以資料表值參數的方式傳送資料
 ...
 針對資料表值呼叫 SQLPutData 時， *DataPtr*會用於此範例中 (可用的資料列數目，一律為 1) 。 *StrLen_or_IndPtr*必須一律為0。 一旦傳遞了資料表值的所有資料列，就會以0的*DataPtr*值來呼叫 SQLPutData。  
...
6.  提供資料執行中參數資料。 當 SQLParamData 傳回資料表值參數的*ParameterValuePtr*時，應用程式必須為數據表值的下一個資料列或資料列準備資料行。 然後，應用程式會呼叫 SQLPutData，並將*DataPtr*設為可用的資料列數 (在此範例中為 1) ， *StrLen_or_IndPtr*設定為0。  
