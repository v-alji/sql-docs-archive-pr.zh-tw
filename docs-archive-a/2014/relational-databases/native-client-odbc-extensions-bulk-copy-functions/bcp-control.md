 ...
 *eOption*  
 ...
 BCPDELAYREADFMT  
 布林值，如果設定為 true，將會導致[bcp_readfmt](bcp-readfmt.md)在執行時讀取。 如果為 false (預設) ，bcp_readfmt 會立即讀取格式檔案。 如果 BCPDELAYREADFMT 為 true，而且您呼叫 bcp_columns 或 bcp_setcolfmt，就會發生順序錯誤。  
 ...
 *iValue*  
 這是指定之*eOption*的值。 *iValue*是整數， (LONGLONG 轉換成 void 指標的) 值，以允許未來擴充至64位值。  
