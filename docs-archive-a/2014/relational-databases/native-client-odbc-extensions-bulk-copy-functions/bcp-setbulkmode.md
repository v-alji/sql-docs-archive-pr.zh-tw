...
|BCP_OUT_NATIVE_TEXT_MODE|指定非字元類型的原生類型和字元類型的 Unicode。<br /><br /> 對應至 BCP.EXE 中的-N 選項，而且如果資料 `BCP_FMT_TYPE` 行類型是 (字串，則為屬性設定為的 bcp_setcolfmt （ `SQLNCHAR` 如果不是字串) 則為預設值）。|  
