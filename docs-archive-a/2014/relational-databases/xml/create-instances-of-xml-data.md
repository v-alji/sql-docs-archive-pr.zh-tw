...
## <a name="type-casting-string-and-binary-instances"></a>類型轉換字串和二進位執行個體  
 您可以將字串 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 資料類型（例如 [**n**] [**var**]**char**、 **[n] text**、 **Varbinary**及**image**）剖析成 `xml` 資料類型，方法是將 (CAST 轉換) 或轉換 (將字串) 轉換成 `xml` 資料類型。 將會檢查不具類型的 XML 以確認它的格式正確。 如果有與類型相關聯的架構 `xml` ，也會執行驗證。 如需詳細資訊，請參閱 [比較具類型的 XML 與不具類型的 XML](compare-typed-xml-to-untyped-xml.md)。  
