...
ms.locfileid: "87687841"
---
# <a name="use-large-clr-udts-ole-db"></a><span data-ttu-id="5b753-102">使用大型 CLR UDT (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="5b753-102">Use Large CLR UDTs (OLE DB)</span></span>
  <span data-ttu-id="5b753-103">此範例會示範如何從結果集中提取具有大型使用者定義型別的資料列。</span><span class="sxs-lookup"><span data-stu-id="5b753-103">This sample shows how to fetch rows with large user-defined types from a result set.</span></span> <span data-ttu-id="5b753-104">如需詳細資訊，請參閱[大型 CLR 使用者定義型別 &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md)。</span><span class="sxs-lookup"><span data-stu-id="5b753-104">For more information, see [Large CLR User-Defined Types &#40;OLE DB&#41;](../native-client/ole-db/large-clr-user-defined-types-ole-db.md).</span></span> <span data-ttu-id="5b753-105">此範例適用於 [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] 或更新版本。</span><span class="sxs-lookup"><span data-stu-id="5b753-105">This sample works with [!INCLUDE[ssKatmai](../../includes/sskatmai-md.md)] or later.</span></span>  
  
## <a name="example"></a><span data-ttu-id="5b753-106">範例</span><span class="sxs-lookup"><span data-stu-id="5b753-106">Example</span></span>  
 <span data-ttu-id="5b753-107">此範例包含兩個專案，</span><span class="sxs-lookup"><span data-stu-id="5b753-107">This sample contains two projects.</span></span> <span data-ttu-id="5b753-108">其中一個專案從 C# 原始程式碼建立組件 (DLL)。</span><span class="sxs-lookup"><span data-stu-id="5b753-108">One project creates an assembly (DLL) from C# source code.</span></span> <span data-ttu-id="5b753-109">這個組件包含 CLR 類型。</span><span class="sxs-lookup"><span data-stu-id="5b753-109">This assembly contains the CLR type.</span></span> <span data-ttu-id="5b753-110">將會在資料庫中加入一個資料表。</span><span class="sxs-lookup"><span data-stu-id="5b753-110">A table will be added to the database.</span></span> <span data-ttu-id="5b753-111">資料表中的資料行屬於組件中所定義的類型，根據預設，此範例將會使用 master 資料庫。</span><span class="sxs-lookup"><span data-stu-id="5b753-111">A column in the table will be of a type defined in the assembly, By default, this sample will use the master database.</span></span> <span data-ttu-id="5b753-112">第二個專案是原生的 C 應用程式，可從資料表讀取資料。</span><span class="sxs-lookup"><span data-stu-id="5b753-112">The second project is a native C application that reads data from the table.</span></span>  
  
 <span data-ttu-id="5b753-113">將第一個 (C#) 程式碼清單編譯成 DLL。</span><span class="sxs-lookup"><span data-stu-id="5b753-113">Compile the first (C#) code listing to a DLL.</span></span>  <span data-ttu-id="5b753-114">然後，將此 DLL 複製到 C 磁碟機的根目錄。</span><span class="sxs-lookup"><span data-stu-id="5b753-114">Then, copy the DLL to the root directory of the C drive.</span></span>  
  
 <span data-ttu-id="5b753-115">執行第二個 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 程式碼清單，以便將組件加入至 master 資料庫。</span><span class="sxs-lookup"><span data-stu-id="5b753-115">Execute the second ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to add the assembly to the master database.</span></span>  
  
 <span data-ttu-id="5b753-116">使用 ole32.lib oleaut32.lib 編譯並執行第三個 (C++) 程式碼清單。</span><span class="sxs-lookup"><span data-stu-id="5b753-116">Compile with ole32.lib oleaut32.lib and execute the third (C++) code listing.</span></span> <span data-ttu-id="5b753-117">這個應用程式會連接到電腦的預設 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體。</span><span class="sxs-lookup"><span data-stu-id="5b753-117">This application connects to your computer's default [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="5b753-118">在某些 Windows 作業系統上，您必須將 (localhost) 或 (local) 變更為 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] 執行個體的名稱。</span><span class="sxs-lookup"><span data-stu-id="5b753-118">On some Windows operating systems, you will need to change (localhost) or (local) to the name of your [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] instance.</span></span> <span data-ttu-id="5b753-119">若要連線到具名執行個體，請將連接字串從 L"(local)" 變更為 L"(local)\\\name"，其中 name 是具名執行個體。</span><span class="sxs-lookup"><span data-stu-id="5b753-119">To connect to a named instance, change the connection string from L"(local)" to L"(local)\\\name" , where name is the named instance.</span></span> <span data-ttu-id="5b753-120">根據預設，[!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express 會安裝至具名執行個體。</span><span class="sxs-lookup"><span data-stu-id="5b753-120">By default, [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] Express installs to a named instance.</span></span> <span data-ttu-id="5b753-121">請確認您的 INCLUDE 環境變數包含的目錄內含 sqlncli.h。</span><span class="sxs-lookup"><span data-stu-id="5b753-121">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="5b753-122">執行第四個 ([!INCLUDE[tsql](../../includes/tsql-md.md)]) 程式碼清單，以便從 master 資料庫中刪除組件。</span><span class="sxs-lookup"><span data-stu-id="5b753-122">Execute the fourth ([!INCLUDE[tsql](../../includes/tsql-md.md)]) code listing to delete the assembly to the master database.</span></span>  
