...
ms.locfileid: "87598838"
---
# <a name="retrieve-data-from-a-filestream-column-using-isequentialstream-ole-db"></a><span data-ttu-id="eaab6-102">使用 ISequentialStream 從 FILESTREAM 資料行擷取資料 (OLE DB)</span><span class="sxs-lookup"><span data-stu-id="eaab6-102">Retrieve Data from a FILESTREAM Column Using ISequentialStream (OLE DB)</span></span>
  <span data-ttu-id="eaab6-103">此範例會範例如何在 ICommandText 介面中使用 ISequentialStream 介面來擷取 Filestream 資料行中的單一記錄。</span><span class="sxs-lookup"><span data-stu-id="eaab6-103">This sample shows uses an ISequentialStream interface in an ICommandText interface to retrieve a single record from a Filestream column.</span></span>  
  
 <span data-ttu-id="eaab6-104">如需有關 filestream 功能的詳細資訊，請參閱[&#40;OLE DB&#41;的 Filestream 支援](../../native-client/ole-db/filestream-support-ole-db.md)。</span><span class="sxs-lookup"><span data-stu-id="eaab6-104">For more information on the filestream feature, see [FILESTREAM Support &#40;OLE DB&#41;](../../native-client/ole-db/filestream-support-ole-db.md).</span></span>  
  
## <a name="example"></a><span data-ttu-id="eaab6-105">範例</span><span class="sxs-lookup"><span data-stu-id="eaab6-105">Example</span></span>  
 <span data-ttu-id="eaab6-106">請確認您的 INCLUDE 環境變數包含的目錄內含 sqlncli.h。</span><span class="sxs-lookup"><span data-stu-id="eaab6-106">Make sure your INCLUDE environment variable includes the directory that contains sqlncli.h.</span></span>  
  
 <span data-ttu-id="eaab6-107">使用下列其中一個範例來建立此範例讀取的來源資料表：</span><span class="sxs-lookup"><span data-stu-id="eaab6-107">Use one of the following samples to create the table that this sample reads from:</span></span>  
  
-   [<span data-ttu-id="eaab6-108">使用繫結至 ICommandText 參數的 ISequentialStream 將資料傳送到 FILESTREAM 資料行 &#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="eaab6-108">Send Data to a FILESTREAM Column Using ISequentialStream Bound to ICommandText Parameter &#40;OLE DB&#41;</span></span>](send-data-to-filestream-isequentialstream-bound-to-icommandtext.md)  
  
-   [<span data-ttu-id="eaab6-109">使用 IRowsetFastUpload 將資料傳送到 FILESTREAM 資料行 &#40;OLE DB&#41;</span><span class="sxs-lookup"><span data-stu-id="eaab6-109">Send Data to a FILESTREAM Column Using IRowsetFastUpload &#40;OLE DB&#41;</span></span>](send-data-to-a-filestream-column-using-irowsetfastupload-ole-db.md)  
  
 <span data-ttu-id="eaab6-110">複製第一個程式碼清單並將它貼入名為 ISSHelper.h 的檔案中。</span><span class="sxs-lookup"><span data-stu-id="eaab6-110">Copy the first code listing and paste it into a file called ISSHelper.h.</span></span>  
  
 <span data-ttu-id="eaab6-111">複製第二個程式碼清單並將它貼入名為 ISSHelper.cpp 的檔案中。</span><span class="sxs-lookup"><span data-stu-id="eaab6-111">Copy the second code listing and paste it into a file called ISSHelper.cpp.</span></span>  
  
 <span data-ttu-id="eaab6-112">複製第三個程式碼清單並將它貼入名為 ICommandDownload.cpp 的檔案中。</span><span class="sxs-lookup"><span data-stu-id="eaab6-112">Copy the third code listing and paste it into a file called ICommandDownload.cpp.</span></span>  
  
 <span data-ttu-id="eaab6-113">編譯 ICommandDownload.cpp、ISSHelper.cpp、ole32.lib 和 oleaut32.lib。</span><span class="sxs-lookup"><span data-stu-id="eaab6-113">Compile ICommandDownload.cpp, ISSHelper.cpp, ole32.lib, and oleaut32.lib.</span></span>  
  
 <span data-ttu-id="eaab6-114">執行此範例時，您必須傳遞一個伺服器名稱 (或 server\instance_name)。</span><span class="sxs-lookup"><span data-stu-id="eaab6-114">When this sample is run, you must pass a server name (or server\instance_name).</span></span>  
