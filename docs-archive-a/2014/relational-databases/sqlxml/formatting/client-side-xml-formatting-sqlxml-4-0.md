...
```  
 ...
 伺服器會執行查詢，並將包含 FirstName 和 LastNamecolumns) 的資料列集 (傳回給用戶端。 中間層接著會將 FOR XML 轉換套用至資料列集，並將 XML 格式傳回到用戶端。  
...
```  
 ...
 由於**用戶端-xml**屬性設定為 1 (true) 在範本中，因此預存程式會在伺服器上執行，而伺服器所傳回的兩個數據行資料列集會在仲介層轉換成 xml，並傳回給用戶端。 (此處僅顯示部分結果)。  
