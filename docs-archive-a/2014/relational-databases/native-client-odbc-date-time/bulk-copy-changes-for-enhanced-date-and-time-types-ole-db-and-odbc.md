...
## <a name="character-data-files"></a>字元資料類型  
 ...
 在原生資料檔中，四個新類型的日期和時間值會表示為其 TDS 標記法，其小數位數為 7 (，因為這是所支援的最大值 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] ，而且 bcp 資料檔不會儲存這些資料行的小數位數) 。 現有 `datetime` 和類型的儲存 `smalldatetime` 或其表格式資料流程 (TDS) 標記法並不會變更。  
...
|目標 --><br /><br /> 寄件者|date|time|smalldatetime|Datetime|datetime2|datetimeoffset|char|wchar|  
|------------------------|----------|----------|-------------------|--------------|---------------|--------------------|----------|-----------|  
