...
3.  [活動監控] 畫面會在活動方格中顯示每個活動的相關資訊。 活動方格會顯示有關每個 DQS 活動的下列資訊：
    ...
    |**目前狀態**|指出活動的目前狀態。 活動狀態是由最後一個計算程序所決定。 它具有下列值：<br /><br /> **執行中**：計算程序正在執行中。<br /><br /> **成功**：在任何計算程序執行之前，狀態都設為 **[成功]**。 同樣地，在計算程序成功結束之後，狀態也設為 **[成功]**。<br /><br /> **失敗**：計算程序失敗。<br /><br /> **已停止**：計算程序已停止。<br /><br /> <br /><br /> 注意：一個活動中可能會有數個計算進程，例如在 [知識探索] 活動) 內 (執行探索程式數次。 因此，狀態可能會在活動存留期間變更許多次。|
    |**DQKB**|用於活動的知識庫名稱。|
