...
# <a name="ssis-catalog"></a>SSIS 目錄
  `SSISDB`目錄是用 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 來處理已部署至伺服器之 (SSIS) 專案的中心點 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 。 例如，您可以設定專案和封裝參數、設定環境以指定封裝的執行值、執行和疑難排解封裝，以及管理 [!INCLUDE[ssISnoversion](../../includes/ssisnoversion-md.md)] 伺服器作業。  
