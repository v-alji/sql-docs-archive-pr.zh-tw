...
|屬性名稱|資料類型|描述|  
|-------------------|---------------|-----------------|  
|AccessMode|整數 (列舉)|一個值，指定目的地如何存取其目的地資料庫。<br /><br /> 此屬性可以有下列其中一個值：<br /><br /> `OpenRowset` (0) -您提供資料表或視圖的名稱。<br /><br /> `OpenRowset from Variable` (1) -您可以提供包含資料表或視圖名稱之變數的名稱。<br /><br /> `OpenRowset Using Fastload` (3) -您提供資料表或視圖的名稱。<br /><br /> `OpenRowset Using Fastload from Variable` (4) -您提供包含資料表或視圖名稱之變數的名稱。<br /><br /> `SQL Command` (2) -您提供 SQL 語句。|  
