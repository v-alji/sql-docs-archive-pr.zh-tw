 ...
 **EntryPoint**  
 將 [!INCLUDE[ssISnoversion](../includes/ssisnoversion-md.md)] 執行階段呼叫的方法指定為指令碼工作程式碼的進入點。 指定的方法必須位於 [!INCLUDE[msCoName](../includes/msconame-md.md)] [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] 應用程式之 TOOLS (VSTA) 專案的 ScriptMain 類別中。 ScriptMain 類別是腳本範本所產生的預設類別。  
