...
# <a name="the-oracle-cdc-service"></a>Oracle CDC 服務
 ...
 Oracle CDC 服務是使用 Oracle CDC 服務組態主控台所建立，或是透過 xdbcdcsvc.exe 程式所內建的命令列介面所定義。 在這兩種情況下，所建立的每個 Oracle CDC 服務都會與單一實例相關聯 [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] (可以使用**AlwaysOn**設定進行叢集或鏡像處理) 而且連接資訊 (連接字串和存取認證) 是服務設定的一部分。  
